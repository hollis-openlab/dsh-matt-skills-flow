...
function CreateFlowForm({ store, t, busy }: { store: FlowUiStore; t: Translate; busy: boolean }): React.ReactElement {
  ...
  return (
    <form className="matt-flow-create-form" onSubmit={event => { event.preventDefault(); void store.create(title, repoRoot, initialContext) }}>
      ...
      <label>{t('initialContext')}<textarea value={initialContext} placeholder={t('initialContextPlaceholder')} onChange={event => setInitialContext(event.target.value)} /></label>
...
function FlowDetail({ flow, t, store, busy, frontier }: { flow: FlowRecord; t: Translate; store: FlowUiStore; busy: boolean; frontier?: import('../domain.ts').FrontierPlan }): React.ReactElement {
  ...
  const [frontierApproved, setFrontierApproved] = React.useState(false)
  React.useEffect(() => { setFrontierApproved(false) }, [flow.id, flow.revision, frontier?.flowRevision])
  ...
  return (
    <>
      ...
      {flow.questions.filter(question => question.status === 'pending').length > 0 ? <section className="matt-flow-panel"><h3>{t('questions')}</h3>{flow.questions.filter(question => question.status === 'pending').map(question => <form className="matt-flow-question-row" key={question.id} onSubmit={event => { event.preventDefault(); const value = questionAnswers[question.id] ?? ''; if (value.trim().length > 0) { void store.answerQuestion(flow, question.id, value); setQuestionAnswers(current => ({ ...current, [question.id]: '' })) } }}><p>{question.question}</p>{question.context ? <small>{question.context}</small> : null}{question.options && question.options.length > 0 ? <ul>{question.options.map(option => <li key={option}>{option}</li>)}</ul> : null}{question.sourceRefs && question.sourceRefs.length > 0 ? <small>{question.sourceRefs.join(', ')}</small> : null}<input value={questionAnswers[question.id] ?? ''} placeholder={t('questionAnswerPlaceholder')} onChange={event => setQuestionAnswers(current => ({ ...current, [question.id]: event.target.value }))} /><button type="submit" disabled={busy || (questionAnswers[question.id] ?? '').trim().length === 0}>{t('answerQuestion')}</button></form>)}</section> : null}

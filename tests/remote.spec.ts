...
describe('Matt Skills Remote', () => {
  it('exposes the Flow and Decision Ledger operations', () => {
    expect(TYPERT_REMOTE.descriptors.map(descriptor => descriptor.method)).toEqual(['list', 'get', 'create', 'advance', 'decide', 'ticket', 'updateTicket', 'startActivity', 'completeActivity', 'rejectAcceptance', 'lane', 'publish', 'provisionLane', 'runLane', 'prepareAcceptance', 'accept', 'cleanup', 'integrate', 'answerQuestion', 'resume', 'previewFrontier', 'startFrontier', 'requestReview', 'disposeFinding', 'generateSpec', 'approveSpec', 'exportEvidence'])
  })
